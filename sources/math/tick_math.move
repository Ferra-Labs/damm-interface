module ferra_damm::tick_math {
    ...
    const E_INVALID_TICK_RANGE: u64 = 3;
    const EINVALID_TICK_SPACING: u64 = 4;
    ...
    public fun validate_tick_range(tick_lower: I32, tick_upper: I32, tick_spacing: u32) {
        let valid = if (i32::lt(tick_lower, tick_upper)) {
            if (i32::gte(tick_lower, min_tick())) {
                if (i32::lte(tick_upper, max_tick())) {
                    if (i32::mod(tick_lower, i32::from(tick_spacing)) == i32::zero()) {
                        i32::mod(tick_upper, i32::from(tick_spacing)) == i32::zero()
                    } else {
                        false
                    }
                } else {
        ...
        assert!(valid, E_INVALID_TICK_RANGE);
    }
